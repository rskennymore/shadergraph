...
fn fragment(in: VertexOutput, @builtin(front_facing) is_front: bool) -> @location(0) vec4<f32> {
    ...
    let n5_colf = dot((sg_vertex_color).rgb, vec3<f32>(0.2126, 0.7152, 0.0722));
    let n7_param = params.slots[0].x;
    let n6_param = params.slots[1].x;
    let n8_math = (n7_param - n6_param);
    let n9_math = min(n5_colf, n8_math);
    let n1_param = params.slots[2].x;
    let n2_vec = (sg_uv * n1_param);
    let n3_voronoi = sg_voronoi_f1f2_3d(n2_vec * 5.0, 1.0);
    let n4_ramp = sg_n4_ramp(n3_voronoi.f1_distance);
    let n10_param = params.slots[18];
    let n11_mix = mix(n4_ramp.rgba, n10_param, clamp(n9_math, 0.0, 1.0));
    let n12_param = params.slots[19].x;
    let n13_param = params.slots[20].x;
    let n14_surface = SgSurface(n11_mix, n12_param, n13_param, 0.5, vec4<f32>(0.0, 0.0, 0.0, 1.0), 1.0, sg_normal, 0.0, vec3<f32>(0.0), 1.0);

    // --- surface properties ---
    pbr_input.material.base_color = n14_surface.base_color;
    pbr_input.material.base_color.a = n14_surface.alpha;
    pbr_input.material.metallic = n14_surface.metallic;
    pbr_input.material.perceptual_roughness = n14_surface.perceptual_roughness;
    pbr_input.material.reflectance = vec3<f32>(n14_surface.reflectance);
    pbr_input.material.emissive = n14_surface.emissive;
    pbr_input.diffuse_occlusion = vec3<f32>(n14_surface.occlusion);
    pbr_input.N = normalize(n14_surface.normal);

    // --- anisotropy ---
    pbr_input.anisotropy_strength = n14_surface.anisotropy_strength;
    let sg_aniso_t = sg_reject_root(n14_surface.anisotropy_tangent, pbr_input.N);
