...
fn sg_n6_ramp(t: f32) -> vec4<f32> {
    let bounds0 = params.slots[9];
    let bounds1 = params.slots[16];
    let bounds2 = params.slots[23];
    if (t <= bounds0.x) { return params.slots[3]; }
    if (t < bounds0.z) {
        let u = clamp((t - bounds0.x) * bounds0.y, 0.0, 1.0);
        return clamp(params.slots[3] + u * (params.slots[4] + u * (params.slots[5] + u * (params.slots[6] + u * (params.slots[7] + u * params.slots[8])))), vec4<f32>(0.0), vec4<f32>(1.0));
    }
    if (t < bounds1.z) {
        let u = clamp((t - bounds1.x) * bounds1.y, 0.0, 1.0);
        return clamp(params.slots[10] + u * (params.slots[11] + u * (params.slots[12] + u * (params.slots[13] + u * (params.slots[14] + u * params.slots[15])))), vec4<f32>(0.0), vec4<f32>(1.0));
    }
    if (t < bounds2.z) {
        let u = clamp((t - bounds2.x) * bounds2.y, 0.0, 1.0);
        return clamp(params.slots[17] + u * (params.slots[18] + u * (params.slots[19] + u * (params.slots[20] + u * (params.slots[21] + u * params.slots[22])))), vec4<f32>(0.0), vec4<f32>(1.0));
    }
    return params.slots[24];
}
...
fn fragment(in: VertexOutput, @builtin(front_facing) is_front: bool) -> @location(0) vec4<f32> {
    ...
    let n3_param = params.slots[1].x;
    let n4_param = params.slots[2].x;
    let n5_voronoi = sg_voronoi_f1f2_3d(n2_vec * n3_param, n4_param);
    let n6_ramp = sg_n6_ramp(n5_voronoi.f1_distance);
    let n7_param = params.slots[25].x;
    let n8_param = params.slots[26].x;
    let n9_surface = SgSurface(n6_ramp.rgba, n7_param, n8_param, 0.5, vec4<f32>(0.0, 0.0, 0.0, 1.0), 1.0, sg_normal, 0.0, vec3<f32>(0.0), 1.0);

    // --- surface properties ---
    pbr_input.material.base_color = n9_surface.base_color;
    pbr_input.material.base_color.a = n9_surface.alpha;
    pbr_input.material.metallic = n9_surface.metallic;
    pbr_input.material.perceptual_roughness = n9_surface.perceptual_roughness;
    pbr_input.material.reflectance = vec3<f32>(n9_surface.reflectance);
    pbr_input.material.emissive = n9_surface.emissive;
    pbr_input.diffuse_occlusion = vec3<f32>(n9_surface.occlusion);
    pbr_input.N = normalize(n9_surface.normal);

    // --- anisotropy ---
    pbr_input.anisotropy_strength = n9_surface.anisotropy_strength;
    let sg_aniso_t = sg_reject_root(n9_surface.anisotropy_tangent, pbr_input.N);
